...
async function exportMarkdown(opts) {
  ...
  // 三者与样式预设正交；页脚蕴含分页（lib/footer.js）
  const fm = parseFrontMatter(file);
  const footerOn =
  ...
  if (preset) {
    ...
    header = (await buildInlineFontCss(preset, fs.readFileSync(file, 'utf8'))) + header;
...
async function exportPdfWithFooter(engine, targetFile, o) {
  ...
  const footerFontCss = o.footer === false ? '' : await buildFooterFontCss(raw);
  full = full
