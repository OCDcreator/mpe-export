...
.mpe-toc-l1 { font-weight: 700; font-size: 13px; margin-top: 0.55em; }
.mpe-toc-title + .mpe-toc-item { margin-top: 0; }
.mpe-toc-l2 { padding-left: 1.2em; }
.mpe-toc-l3 { padding-left: 2.4em; font-size: 11px; }
.mpe-toc-item-title {
    flex: 0 1 auto;
    min-width: 0;
    overflow: hidden;
...
(function () {
  ...
  function isSplittableContainer(block) {
    ...
    return tag === 'BLOCKQUOTE' || tag === 'UL' || tag === 'OL' || tag === 'TABLE' ||
    ...
    while (changed && guard < 200) {
      ...
      for (var i = 0; i < sheets.length - 1; i++) {
        ...
        reflow.forEach(function (blk) {
          ...
          cur = startSheetWithPulled(root, cur, blk);
        });
        changed = true;
        break; // 页框列表已重建，从头再扫
      }
  ...
  async function paginate() {
    ...
    for (var i = 0; i < blocks.length; i++) {
      ...
      sheet = startSheetWithPulled(root, sheet, blk);
    }
